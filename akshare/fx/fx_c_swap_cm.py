...
def fx_c_swap_cm():
    ...
    try:
        ...
        raise RuntimeError(f"ChinaMoney C-Swap endpoint request failed: {url}") from exc
    if r.status_code != 200:
        raise RuntimeError(
            f"ChinaMoney C-Swap endpoint returned HTTP {r.status_code}: {url}"
        )
    try:
        data_json = r.json()
    except ValueError as exc:
        raise RuntimeError(
            f"ChinaMoney C-Swap endpoint returned non-JSON response: {url}; "
